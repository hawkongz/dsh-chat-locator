...
- **曲线长度渐变：** 预览开启时，指针下那根长到 32px，两侧沿曲线收回 —— `21 / 14 / 12` —— 整条轨道读起来是一个指向你所在位置的弯钩，而不是一截直斜线。
...
| 悬停预览 | 开 | 开 / 关 | 悬停时浮出该轮的提示词与回复。它同时决定长度渐变是否生效 —— 关闭预览就不套渐变。 |
...
见 [CONTRIBUTING.md](../CONTRIBUTING.md)。**没有任何东西需要安装**：克隆仓库后直接跑 `node test/verify-client.mjs`，就能拿到全部 121 项断言。
