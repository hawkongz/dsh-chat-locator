...
想锁死某个版本、不跟新版本走，就带上版本号：`dsh plugin --profile web add dsh-chat-locator@1.4.0`

装完就完了。这个包就是一个 DSH 插件 bundle 需要的四个文件 —— `package.json`（声明 bundle 与浏览器半侧）、`index.js`（宿主半侧，1.4.0 起是空桩）、`client.js`（浏览器半侧）、`cordis.patch.yml`（把插件行插进配置树的 patch）—— 没有东西要编译，也没有依赖要装。
...
| 方式 | 适合谁 |
| :--- | :--- |
| `dsh plugin --profile web add dsh-chat-locator` | 大多数人。从 npm 装已发布的版本；带 `@1.4.0` 可以锁死具体版本。 |
...
紧挨悬停那格掉得最多，之后越来越缓，最后贴着轨道收尾。宽度表由 `12 + 20 * (1 - d/3)^2` 算出，幂次是怎么调出来的记在 [设计说明](docs/design-notes.md)里（2.5 像断崖，1.5 邻居偏长，2.0 是现在用的折中）。渐变锚在悬停那根 —— 当前轮的刻度宽度不被改写 —— 指针移开即恢复。与悬停预览不同（预览没有开关：不能预览的轨道就不是轨道了），长度渐变纯属装饰，设置页里有它独立的一行；关掉它同时也会撤掉框架加宽 —— 没有 32px 尖端要画，就不该白添那条隐形交互带。
...
见 [CONTRIBUTING.md](../CONTRIBUTING.md)。**没有任何东西需要安装**：克隆仓库后直接跑 `node test/verify-client.mjs`，就能拿到全部 125 项断言。
